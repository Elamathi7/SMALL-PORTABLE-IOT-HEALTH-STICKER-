...
const char* ssid = "vivo Y21";  // Enter SSID here
const char* password = "999999999";  // Enter Password here
...
// MAX30100 register addresses for temperature
#define MAX30100_TEMP_INTG_REG 0x1F
#define MAX30100_TEMP_FRAC_REG 0x20

// Function prototypes
String SendHTML(float BPM, float SpO2, float temperature);
...
void loop() {
  ...
  if (millis() - tsLastReport > REPORTING_PERIOD_MS) {
    ...
    SpO2 = pox.getSpO2();
    temperature = readTemperature(); // Retrieve temperature
...
void handle_OnConnect() {
  server.send(200, "text/html", SendHTML(BPM, SpO2, temperature));
}
...
float readTemperature() {
  ...
  Wire.beginTransmission(0x57); // Address of the MAX30100 sensor
  Wire.write(MAX30100_TEMP_INTG_REG);
  ...
  Wire.write(MAX30100_TEMP_FRAC_REG);
  Wire.endTransmission();
  Wire.requestFrom(0x57, 1);
  if (Wire.available()) {
    tempFrac = Wire.read();
...
String SendHTML(float BPM, float SpO2, float temperature) {
  ...
  ptr += "<h3><a href='SMART PORTABLE IOT HEALTH STICKER'>SMART PORTABLE IOT HEALTH STICKER</a></h3>";
  ptr += "</div>";
  ...
  // For Temperature
  ptr += "<p class='sensor'>";
  ptr += "<i class='fas fa-thermometer-half' style='color:#ffcc00'></i>";
